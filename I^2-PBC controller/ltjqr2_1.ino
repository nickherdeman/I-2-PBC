...
// 声明一个IIR滤波器对象
const uint8_t NB = 3; // 分子阶数
const uint8_t NA = 3; // 分母阶数
const double b_coefficients[NB] = {0.02008337, 0.04016673, 0.02008337}; // 分子系数数组
const double a_coefficients[NA] = {1.0, -1.56101808, 0.64135154}; // 分母系数数组
int time_flag;
float ea,eb;

// 使用makeIIRFilter函数创建IIR滤波器对象
 IIRFilter<3, 3, float> filter = {{0.02008337, 0.04016673, 0.02008337}, {1.0, -1.56101808, 0.64135154}};
...
VectorInt16 last_gy;         // [x, y, z]            gyro sensor measurements
...
unsigned char angle_flag = 0;//区别电机速度和角度
...
float Pitch_vel_mid1 = 0;

float Amotor_speed = 0;   //A电机当前速度
float Amotor_speed_mid1 = 0;
float last_Amotor_speed = 0;  
float Amotor_speed_filtered = 0;   //A电机当前速度
float last_Amotor_speed_filtered = 0;
float Amotor_acc = 0;   //A电机当前速度

float Bmotor_speed = 0;
float Bmotor_speed_mid1 = 0;
float last_Bmotor_speed = 0;
float Bmotor_speed_filtered = 0;
float last_Bmotor_speed_filtered = 0;
float Bmotor_acc = 0;

float Amotor_angle = 0;
float Bmotor_angle = 0;
float Amotor_angle_prev = 0;  //电机上一次的角度
float Bmotor_angle_prev = 0;

float Pitch_vel_filtered = 0;
float last_Pitch_vel_filtered = 0;
...
uint8_t n_sample4 = 8;
float sample_array4[8] = {0};

uint8_t n_sample5 = 8;
float sample_array5[8] = {0};

uint8_t n_sample6 = 8;//差分得到加速度滤波
float sample_array6[8] = {0};

uint8_t n_sample7 = 8;//pitch滤波
float sample_array7[8] = {0};

uint8_t n_sample8 = 8;//pitch滤波
float sample_array8[8] = {0};
...
}A_speed,B_speed,Zturn, A_speed_true, B_speed_true,Pitch_vel;
...
void AB_Velocity_filter_init()
{
  ...
  A_speed_true.Q_Xk = 0.01f;
  A_speed_true.R_measure = 0.001f;
  A_speed_true.Xk = 0.0f;
  A_speed_true.p1 = 0.01f;

  B_speed_true.Q_Xk = 0.01f;
  B_speed_true.R_measure = 0.001f;
  B_speed_true.Xk = 0.0f;
  B_speed_true.p1 = 0.01f;

    
  Pitch_vel.Q_Xk = 0.1f;
  Pitch_vel.R_measure = 0.01f;
  Pitch_vel.Xk = 0.0f;
  Pitch_vel.p1 = 0.01f;
...
void A_Speed_filter(float NewSpeed )
{
  A_speed_true.p1 = A_speed_true.p1 + A_speed_true.Q_Xk;
  float S = A_speed_true.p1 + A_speed_true.R_measure;
  float k1 = A_speed_true.p1 / S;
  A_speed_true.Xk = A_speed_true.Xk + k1 * (NewSpeed - A_speed_true.Xk);
  A_speed_true.p1 = (1 - k1) * A_speed_true.p1;
}


void B_Speed_filter(float NewSpeed )
{
  B_speed_true.p1 = B_speed_true.p1 + B_speed_true.Q_Xk;
  float S = B_speed_true.p1 + B_speed_true.R_measure;
  float k1 = B_speed_true.p1 / S;
  B_speed_true.Xk = B_speed_true.Xk + k1 * (NewSpeed - B_speed_true.Xk);
  B_speed_true.p1 = (1 - k1) * B_speed_true.p1;
}


void Pitch_vel_filter(float NewSpeed )
{
  Pitch_vel.p1 = Pitch_vel.p1 + Pitch_vel.Q_Xk;
  float S = Pitch_vel.p1 + Pitch_vel.R_measure;
  float k1 = Pitch_vel.p1 / S;
  Pitch_vel.Xk = Pitch_vel.Xk + k1 * (NewSpeed - Pitch_vel.Xk);
  Pitch_vel.p1 = (1 - k1) * Pitch_vel.p1;
}
...
void YKalmangGetAngle(float newAngle, float newRate, float dt) {//实现一个卡尔曼滤波
...
void pid_init()  //pid初始化变量
{
  ...
  Avelocity.Kp = 0.3; //0.3
  Avelocity.Ki = 0.4; //0.4
  Avelocity.Kd = 0; 
  Avelocity.newerr = 0;
  Avelocity.newerr1 = 0;
  ...
  Bvelocity.Kp = 0.3;  //0.3
  Bvelocity.Ki = 0.4;   //0.4
  Bvelocity.Kd = 0; 
  Bvelocity.newerr = 0;
  Bvelocity.newerr1 = 0;  
}
...
float A_speed_Sliding_weighted_filter(float xdat4) //滑动加权滤波算法
{
  long array_sum = 0; //采样队列和
  for (int i = 1; i < n_sample4; i++)
  {
    sample_array4[i - 1] = sample_array4[i];
    array_sum += sample_array4[i] * i;
  }
  sample_array4[n_sample4 - 1] = xdat4;
  array_sum += xdat4 * n_sample4;
  float filte_value = (array_sum / (11 * n_sample4 / 2.0)) * 9 / 7.0; //
  return filte_value;
}


float B_speed_Sliding_weighted_filter(float xdat5) //滑动加权滤波算法
{
  long array_sum = 0; //采样队列和
  for (int i = 1; i < n_sample5; i++)
  {
    sample_array5[i - 1] = sample_array5[i];
    array_sum += sample_array5[i] * i;
  }
  sample_array5[n_sample5 - 1] = xdat5;
  array_sum += xdat5 * n_sample5;
  float filte_value = (array_sum / (11 * n_sample5 / 2.0)) * 9 / 7.0; //
  return filte_value;
}



float A_acc_Sliding_weighted_filter(float xdat6) //滑动加权滤波算法
{
  long array_sum = 0; //采样队列和
  for (int i = 1; i < n_sample6; i++)
  {
    sample_array6[i - 1] = sample_array6[i];
    array_sum += sample_array6[i] * i;
  }
  sample_array6[n_sample6 - 1] = xdat6;
  array_sum += xdat6 * n_sample6;
  float filte_value = (array_sum / (11 * n_sample6 / 2.0)) * 9 / 7.0; //
  return filte_value;
}



float Pitch_vel_Sliding_weighted_filter(float xdat7) //滑动加权滤波算法
{
  long array_sum = 0; //采样队列和
  for (int i = 1; i < n_sample7; i++)
  {
    sample_array7[i - 1] = sample_array7[i];
    array_sum += sample_array7[i] * i;
  }
  sample_array7[n_sample7 - 1] = xdat7;
  array_sum += xdat7 * n_sample7;
  float filte_value = (array_sum / (11 * n_sample7 / 2.0)) * 9 / 7.0; //
  return filte_value;
}



float Pitch_acc_Sliding_weighted_filter(float xdat8) //滑动加权滤波算法
{
  long array_sum = 0; //采样队列和
  for (int i = 1; i < n_sample8; i++)
  {
    sample_array8[i - 1] = sample_array8[i];
    array_sum += sample_array8[i] * i;
  }
  sample_array8[n_sample8 - 1] = xdat8;
  array_sum += xdat8 * n_sample8;
  float filte_value = (array_sum / (11 * n_sample8 / 2.0)) * 9 / 7.0; //
  return filte_value;
}
...
void Read_motor_speed()//需要理解的一个函数
{         
  while (Serial1.available() > 0) 
  {
     ...
     else if((ccnt==1)&&((dat == 66) ||(dat == 55) ))
     {
       rxbuf[ccnt] = dat;
       recstatu = 1;
       ccnt = 2; 
       if (dat == 66) angle_flag=0;
       else angle_flag=1;
     }//接受数据的前两位是一个标志位置
          ...
          if(ccnt==7)
          {
              if(crc1(rxbuf))
              {
                ...
                if(angle_flag == 0)
                {
                  ...
                  Amotor_speed_mid1 = Amotor_speed*0.3+ Amotor_speed_mid1 *0.7;
                  Amotor_speed_filtered = A_speed_Sliding_weighted_filter(Amotor_speed_mid1);
                  A_Speed_filter(Amotor_speed_filtered);
                  Amotor_speed_filtered = A_speed_true.Xk;
                  
                  Bmotor_speed = -((x2-32767)*0.01);
                  Bmotor_speed = -((x2 - 32767) * 0.01);
                  Bmotor_speed_mid1 = Bmotor_speed*0.3+ Bmotor_speed_mid1 *0.7;
                  Bmotor_speed_filtered = B_speed_Sliding_weighted_filter(Bmotor_speed_mid1);
                  B_Speed_filter(Bmotor_speed_filtered);
                  Bmotor_speed_filtered = B_speed_true.Xk;

                  
                  Amotor_acc = A_acc_Sliding_weighted_filter( (Amotor_speed_filtered - last_Amotor_speed_filtered) / Ts );
                  Bmotor_acc = A_acc_Sliding_weighted_filter( (Bmotor_speed_filtered - last_Bmotor_speed_filtered) / Ts );
                  last_Amotor_speed = Amotor_speed;
                  last_Bmotor_speed = Bmotor_speed;
                  last_Amotor_speed_filtered = Amotor_speed_filtered;
                  last_Bmotor_speed_filtered = Bmotor_speed_filtered;
                }
                else
                {
                  Amotor_angle = (x1-32767)*0.01;
                  Bmotor_angle = (x2-32767)*0.01;
...
{ 
  ...
  if(key2)
  {
   ...
   {
    case 0:
//            SerialBT.write((uint8_t*)&car_ch3, sizeof(car_ch3));
//            SerialBT.write(reinterpret_cast<const uint8_t*>("car_ch3"), strlen("car_ch3"));
//            SerialBT.write(reinterpret_cast<const uint8_t*>(String(car_ch3).c_str()), String(car_ch3).length());
            SerialBT.write(reinterpret_cast<const uint8_t*>(String(KalmanY.Angle-AngleY_bias).c_str()), String(KalmanY.Angle-AngleY_bias).length());//pitch角度
            SerialBT.write('\t');
            SerialBT.write(reinterpret_cast<const uint8_t*>(String((float)(-gy.y/16.4)).c_str()), String((float)(-gy.y/16.4)).length());//pitch角速度
            SerialBT.write('\t');
            SerialBT.write(reinterpret_cast<const uint8_t*>(String(acc_p).c_str()), String(acc_p).length());//pitch加速度
            SerialBT.write('\t');
            SerialBT.write(reinterpret_cast<const uint8_t*>(String(acc_IIR).c_str()), String(acc_IIR).length());//pitch加速度滤波之后的
            SerialBT.write('\t');
            SerialBT.write(reinterpret_cast<const uint8_t*>(String(Amotor_speed).c_str()), String(Amotor_speed).length());//电机速度
            SerialBT.write('\t');
            SerialBT.write(reinterpret_cast<const uint8_t*>(String(Abalance.ActualOut).c_str()), String(Abalance.ActualOut).length());//电机设定速度
            SerialBT.write('\t');
            SerialBT.write(reinterpret_cast<const uint8_t*>(String(Amotor_angle).c_str()), String(Amotor_angle).length());//电机角度
            SerialBT.write('\t');
            SerialBT.write(reinterpret_cast<const uint8_t*>(String(Amotor_acc).c_str()), String(Amotor_acc).length());//电机角加速度
            SerialBT.write('\t');
            SerialBT.write(reinterpret_cast<const uint8_t*>(String(KalmanX.Angle-AngleX_bias).c_str()), String(KalmanX.Angle-AngleX_bias).length());//row角度
            SerialBT.write('\t');
            SerialBT.write(reinterpret_cast<const uint8_t*>(String((float)(-gy.z/16.4)).c_str()), String((float)(-gy.z/16.4)).length());//yaw角速度
            SerialBT.write('\t');

            SerialBT.write(reinterpret_cast<const uint8_t*>(String(Ts).c_str()), String(Ts).length());//周期
            ...
            Serial.print("\t"); 
            Serial.print(Pitch_vel_filtered);
            Serial.print("\t"); 
            Serial.print(acc_IIR);
            Serial.print("\t"); 
            Serial.print(Amotor_speed_filtered);
            Serial.print("\t"); 
            Serial.print(Amotor_angle);
            Serial.print("\t"); 
            Serial.print(Amotor_acc);
            Serial.print("\t"); 
            Serial.print(KalmanX.Angle-AngleX_bias);
            Serial.print("\t");            
            Serial.print(-gy.z/16.4);
            Serial.print("\t");         
            Serial.print(Apwm1);
            Serial.print("\t"); 
            Serial.print(wa);
...
{  
   ...
   Pitch_vel_mid1 = 0.3*y1+0.7*Pitch_vel_mid1;
   Pitch_vel_filtered = Pitch_vel_Sliding_weighted_filter(Pitch_vel_mid1);
   Pitch_vel_filter(Pitch_vel_filtered);
   Pitch_vel_filtered = Pitch_vel.Xk;




    float y2 = (float)(-last_gy.y/16.4);
    float x2 = (float)(last_gy.x/16.4);
    float z2 = (float)(-last_gy.z/16.4);

    acc_p =(Pitch_vel_filtered - last_Pitch_vel_filtered) / Ts;
    

    acc_IIR = Pitch_acc_Sliding_weighted_filter(acc_p);
    last_gy=gy;
    last_Pitch_vel_filtered = Pitch_vel_filtered;
...
{
  Abalance.Kp = 2.6;//0.00001*sq(ah)+0.0068*ah+0.1028-0.4;   //0.00001*sq(ah)+0.0068*ah+0.1028  // 2.6
  Abalance.Ki = 0;//0.00002*sq(ah)+0.0396*ah+0.8171-0.3;     // 0.00002*sq(ah)+0.0396*ah+0.8171  //0
  Abalance.Kd = 0;//0; //0.001                                                              //0.0005

  Avelocity.Kp = 0.22;//0.000002*sq(ah)-0.002*ah+0.539-0.15;    //0.000002*sq(ah)-0.002*ah+0.539 //0.215
  Avelocity.Ki = -0.002*ah+1.1 ;//-0.002*ah+0.6857-0.15;  //-0.002*ah+0.6857                                //0.45 0.8
  Avelocity.Kd = 0;//0;   //0.0001                                                                //0

  Bbalance.Kp = 2.6;//0.00001*sq(ah)+0.0068*ah+0.1028-0.4;   //0.00001*sq(ah)+0.0068*ah+0.1028 //2.6
  Bbalance.Ki = 0;//0.00002*sq(ah)+0.0396*ah+0.8171-0.3;  //0.00002*sq(ah)+0.0396*ah+0.8171     //0
  Bbalance.Kd = 0;//0;  //0.001                                                           //0.0005

  Bvelocity.Kp = 0.22;//0.000002*sq(ah)-0.002*ah+0.539-0.15;   //0.000002*sq(ah)-0.002*ah+0.539   //0.215
  Bvelocity.Ki = -0.002*ah+1.1;//-0.002*ah+0.6857-0.15;    //-0.002*ah+0.6857                                //0.45
  Bvelocity.Kd = 0;//0;   //0.0001                                                                  //0 0.8
...
void PID_parameterx(float ah,float bh)
{
  Abalance.Kp = 0.00001*sq(ah)+0.0068*ah-0.05;   //0.00001*sq(ah)+0.0068*ah+0.1028  
  Abalance.Ki = 0.00002*sq(ah)+0.0396*ah+0.8;     // 0.00002*sq(ah)+0.0396*ah+0.8171
  Abalance.Kd = 0; 

  Avelocity.Kp = 0.000002*sq(ah)-0.002*ah+0.48;    //0.000002*sq(ah)-0.002*ah+0.539 
  Avelocity.Ki = -0.002*ah+0.75;  //-0.002*ah+0.6857
  Avelocity.Kd = 0;   

  Bbalance.Kp = 0.00001*sq(ah)+0.0068*ah+0.1028;   //0.00001*sq(ah)+0.0068*ah+0.1028 
  Bbalance.Ki = 0.00002*sq(ah)+0.0396*ah+0.8;  //0.00002*sq(ah)+0.0396*ah+0.8171
  Bbalance.Kd = 0; 

  Bvelocity.Kp = 0.000002*sq(ah)-0.002*ah+0.48;   //0.000002*sq(ah)-0.002*ah+0.539   
  Bvelocity.Ki = -0.002*ah+0.75;    //-0.002*ah+0.6857
  Bvelocity.Kd = 0;   
...
float IDA_PBC(float V_d,float pitch,float Aspeed,float Bspeed,float omega1)
{
  Km=1;
 ...
 float kp=0.9;
  ...
  theta=3.14159/180*pitch;
  u1= (kd*Avelocity.err*(cos(theta) + 5))/(5*Km);
  u2= (kp*pitch*(cos(theta) + 5)*(cos(theta) + 5))/(25*Km);//+4*er;
  u3=(2225*Km*pp*(ep)*(2300*cos(theta) + 84*cos(theta)*cos(theta) - 21245))/((cos(theta) + 5)*(cos(theta) + 5)*(1764*cos(theta)*cos(theta) - 306605));//+2*epi;
  u=u1+u2-u3;
  return u*100;
}
...
float IDA_PBC1(float V_d,float pitch,float Aspeed,float Bspeed,float omega1) 
{   
    ...
    if(er<(-2.5)) er = -2.5;   
     ...
     u=(6*kd*(Avelocity.err))/(5*Km) - (13988575*Km*pp*ep)/3658092 - (36*kd*omega1)/(25*Km)-2*epi+1.3*pitch+3*er;
...
float IDA_PBC2(float V_d,float pitch,float Aspeed,float Bspeed,float omega1)  
{   
    ...
    u1 = Avelocity.Kp*(cos(theta) + 5)*Avelocity.err/(5*Km) + (2225*Km*pp*(ep)*(2300*cos(theta) + 84*cos(theta)*cos(theta) - 21245))/((cos(theta) + 5)*(cos(theta) + 5)*(1764*cos(theta)*cos(theta) - 306605))-Amotor_acc/200;
    if(u1>22) u1 = 22;   
    if(u1<(-22)) u1 = -22;
    ...
    if(er<(-2.5)) er = -2.5;  
     ...
     u2 =  Abalance.Kp*pr+5*er-acc_IIR/2000;
 ...
 //u2= (kp*pitch*(cos(theta) + 5)*(cos(theta) + 5))/(25*Km);//+4*er;
 // u3=(2225*Km*pp*(ep)*(2300*cos(theta) + 84*cos(theta)*cos(theta) - 21245))/((cos(theta) + 5)*(cos(theta) + 5)*(1764*cos(theta)*cos(theta) - 306605));//+2*epi;
   //u=Abalance.Kp*(pitch-constrain(Avelocity.Ki*constrain(ep,-30,30)+Avelocity.Kp*Avelocity.err,-22,22))+Abalance.Ki*constrain(er,-2.5,2.5);
}
...
void loop() 
{
  ...
  {      
      ...
      { 
       ...
       // int dwa=(int)(Abalance_pin(asd,(KalmanY.Angle-AngleY_bias)));
       // int dwb=(int)(Abalance_pin(asd,(KalmanY.Angle-AngleY_bias)));
       // int Apwm = dwa*100-Tpwm;
        //int Bpwm = dwb*100+Tpwm;
        ...
        // coupled INDI  1
        //float asd = Avelocity_pin(car_ch2,Amotor_speed,Bmotor_speed);//-Amotor_acc/Abalance.Kp; // /20
        //float bsd = Bvelocity_pin(car_ch2,Amotor_speed,Bmotor_speed);//-Bmotor_acc/Bbalance.Kp;
        // dwa=(int)(Abalance_pin(asd,(KalmanY.Angle-AngleY_bias))-acc_IIR);
        // dwb=(int)(Abalance_pin(asd,(KalmanY.Angle-AngleY_bias))-acc_IIR);
        // wa=wa+dwa;
        // wb=wb+dwb;
        //int Apwm = wa*100-Tpwm;
        //int Bpwm = wb*100+Tpwm;


        // coupled INDI   2
        //float asd = Avelocity_pin(car_ch2,Amotor_speed,Bmotor_speed);//-Amotor_acc/Abalance.Kp/20; // /20
        //float bsd = Bvelocity_pin(car_ch2,Amotor_speed,Bmotor_speed);//-Bmotor_acc/Bbalance.Kp/20;
        //int dwa1=(int)(Abalance_pin(asd,(KalmanY.Angle-AngleY_bias)));//-acc_IIR/20);
        //int dwb1=(int)(Abalance_pin(asd,(KalmanY.Angle-AngleY_bias)));//-acc_IIR/20);
        //int wa1=wa1+dwa1;
        //int wb1=wb1+dwb1;
        //int Apwm = dwa1*100-Tpwm;
        //int Bpwm = dwb1*100+Tpwm;


       // IDA-PBC
       int dwa1=(int)(IDA_PBC2(car_ch2,KalmanY.Angle-AngleY_bias,Amotor_speed,Bmotor_speed,Pitch_vel_filtered))*100-Tpwm;
       int dwb1=(int)(IDA_PBC2(car_ch2,KalmanY.Angle-AngleY_bias,Amotor_speed,Bmotor_speed,Pitch_vel_filtered))*100+Tpwm;
       int wa1=wa1+dwa1;
       int wb1=wb1+dwb1;
       int Apwm = dwa1;
       int Bpwm = dwb1;
        ...
        // decoupled  INDI
        //float dab=(int)(Abalance_pin(0,(KalmanY.Angle-AngleY_bias)))/0.0025-acc_IIR/100;
       // float dav=-Abalance.Kp*(Avelocity_pin(car_ch2,Amotor_speed,Bmotor_speed))/0.017-Amotor_acc/100;
        //float dwa=(0.0025*dab+0.017*dav);
       // float wa=wa+dwa;
       // int Apwm = wa*100-Tpwm;

       // float dbb=(int)(Bbalance_pin(0,(KalmanY.Angle-AngleY_bias)))/0.0025-acc_IIR;
       // float dbv=-Bbalance.Kp*(Bvelocity_pin(car_ch2,Amotor_speed,Bmotor_speed))/0.017-Bmotor_acc;
       // float dwb=(0.0025*dbb+0.017*dbv);
       // float wb=wb+dwb;
       // int Bpwm = wb*100+Tpwm;
        ...
        //ea+= (Avelocity_pin(car_ch2,Amotor_speed,Bmotor_speed))*0.01; abalancekp0.09
       // int dav=-Abalance.Kp*(Avelocity_pin(car_ch2,Amotor_speed,Bmotor_speed))-Abalance.Ki*ea;
       // int dab=(int)(Abalance_pin(0,(KalmanY.Angle-AngleY_bias)));
       // int dwa=dab+dav;
       //int Apwm = dwa*100-Tpwm;
       

       
       //  eb+= (Bvelocity_pin(car_ch2,Amotor_speed,Bmotor_speed))*0.01;
       // int dbv=-Bbalance.Kp*(Bvelocity_pin(car_ch2,Amotor_speed,Bmotor_speed))-Bbalance.Ki*eb;
       // int dbb=(int)(Bbalance_pin(0,(KalmanY.Angle-AngleY_bias)));
       // int dwb=dbb+dbv;
       // int Bpwm = dwb*100+Tpwm;
